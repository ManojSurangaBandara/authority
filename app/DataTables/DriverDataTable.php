    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($row) {
                ...
                // Build usage reason
                $reasonText = $isUsed ? "Driver has {$activeAssignmentsCount} active assignment(s)" : '';

                // View button (always available)
                $viewBtn = '<a href="' . route('drivers.show', $row->id) . '" class="btn btn-xs btn-info" title="View"><i class="fas fa-eye"></i></a>';

                // Edit button (always enabled, but with warning tooltip if driver is in use)
                if ($isUsed) {
                    $editBtn = '<a href="' . route('drivers.edit', $row->id) . '" class="btn btn-xs btn-primary mx-1" title="Edit (Note: Regiment number cannot be changed - ' . $reasonText . ')" data-toggle="tooltip"><i class="fas fa-edit"></i></a>';
                } else {
                    $editBtn = '<a href="' . route('drivers.edit', $row->id) . '" class="btn btn-xs btn-primary mx-1" title="Edit"><i class="fas fa-edit"></i></a>';
                }

                // Delete button (disabled if driver is in use)
                if ($isUsed) {
