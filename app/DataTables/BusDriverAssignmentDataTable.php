...
class BusDriverAssignmentDataTable extends DataTable
{
    /**
     * Build the DataTable class.
     *
     * @param QueryBuilder<BusDriverAssignment> $query Results from query() method.
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('driver_details', function ($row) {
                return '<strong>' . $row->driver_regiment_no . '</strong><br>' .
            ...
            ->addColumn('action', function ($row) {
                $viewBtn = '<a href="' . route('bus-driver-assignments.show', $row->id) . '" class="btn btn-xs btn-info" title="View"><i class="fas fa-eye"></i></a>';
                $editBtn = '<a href="' . route('bus-driver-assignments.edit', $row->id) . '" class="btn btn-xs btn-primary mx-1" title="Edit"><i class="fas fa-edit"></i></a>';
                $deleteBtn = '<form action="' . route('bus-driver-assignments.destroy', $row->id) . '" method="POST" style="display:inline">
                    ' . csrf_field() . '
                    ' . method_field("DELETE") . '
            ...
            ->rawColumns(['driver_details', 'status', 'action'])
            ->setRowId('id');
    }
     ...
     *
     * @return QueryBuilder<BusDriverAssignment>
     */
    public function query(BusDriverAssignment $model): QueryBuilder
    {
        return $model->with(['busRoute.bus'])->newQuery();
    }
    ...
    {
        return $this->builder()
            ->setTableId('busdriverassignment-table')
    ...
    {
        return 'BusDriverAssignment_' . date('YmdHis');
    }
}
